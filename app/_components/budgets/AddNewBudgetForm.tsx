...
import { createNewBudget } from "@/app/_actions/budgets/createNewBudget";
import { BudgetType } from "@/models/Budget";

type Category = TransactionType["category"];

type AddNewBudgetFormProps = {
  budgets: BudgetType[];
  usedColor: string[];
  close?: () => void;
};

function AddNewBudgetForm({
  close,
  usedColor,
  budgets,
}: AddNewBudgetFormProps) {
  ...
  const isUsedCategory = usedCategories.some((cat) => cat === category);
  const colorTag = [
  ...
  function handleSubmitAddBudget(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    ...
    startTransition(async () => {
      ...
      const result = await createNewBudget(newBudget);
  ...
  return (
    <>
      {close && <FormHeader close={close}>Add New Budget</FormHeader>}
      <FormDescription>
        Choose a category to set a spending budget. These categories can help
        you monitor spending.
      </FormDescription>
      <form onSubmit={handleSubmitAddBudget} className="flex flex-col gap-200">
        ...
        <Button isPending={isPending}>Add Budget</Button>
      </form>
    </>
  );
}
