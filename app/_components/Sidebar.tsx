  ...
  return (
    ...
    >
      ...
      {navs.map((nav) => (
        ...
        >
          ...
          >
            {nav.icon}
            <p
              className={`${nav.url === pathname ? "text-grey-900" : "text-grey-300"} text-preset-5-bold hidden md:block`}
            >
              {nav.name}
            </p>
          </div>
      ...
      >
        ...
        <p className="text-preset-3 text-grey-300">Minimize Menu</p>
      </div>
    </div>
  );
}
